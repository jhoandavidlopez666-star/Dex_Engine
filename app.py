...
if not st.session_state.autenticado:
    ...
    if st.button("Verificar"):
        if clave == CODIGO_DAVID:
            ...
            st.session_state.nombre = "David López" # Identidad fijada
            st.rerun()
        else:
            st.error("Acceso denegado. Código no reconocido.")
    st.stop()

# --- 2. ACCESO AUTORIZADO (RECONOCIMIENTO) ---
st.success(f"Bienvenido, {st.session_state.nombre}. Sistema operativo vinculado.")
...
# --- 3. LÓGICA DE MANDO CON IDENTIDAD ---
if prompt := st.chat_input(f"¿Qué orden tienes para mí, {st.session_state.nombre}?"):
    if "abre" in prompt.lower() or "abrir" in prompt.lower():
        try:
            ...
            res = f"Entendido, {st.session_state.nombre}. Ejecutando comando en tu dispositivo."
        except:
            res = "Falla de comunicación con el móvil."
    else:
